...
Build a machine learning model that predicts ionic conductivity (log₁₀ σ in S/cm at 300 K) for lithium solid electrolytes, to serve as the surrogate model for the BREED genetic algorithm. The target is to beat the published OBELiX RF baseline (MAE 1.531) — ideally reaching MAE < 0.7, which would be publishable as a feature-engineering contribution.
